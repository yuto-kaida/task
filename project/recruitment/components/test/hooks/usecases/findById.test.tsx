...
import { IdNotFound } from "@/domains/types/errors";
...
describe("useFindById", () => {
  ...
  describe("パスパラメータにidが含まれている", () => {
    ...
    test("パスパラメータではなくKeyで取得、結果が返却される", async () => {
      ...
      await act(async () => {
        result = renderHook(
          () => useFindById(mockFindByID, newTestModel, "key"), // keyを付与する
        ).result;
        await Promise.resolve();
      });
  ...
  describe("パスパラメータからidが取得できない", () => {
    ...
    test("引数が足りないのでapiはコールされない", async () => {
      ...
      expect(result.current.isLoading).toBe(false);
      expect(result.current.error).toEqual(new Error(IdNotFound));
      expect(result.current.hasFailed).toBe(true);
    ...
    test("keyが指定されているとapiはコールされる", async () => {
      ...
      await act(async () => {
        result = renderHook(() =>
          useFindById(mockFindByID, newTestModel, "key"),
        ).result;
        await Promise.resolve();
      });
